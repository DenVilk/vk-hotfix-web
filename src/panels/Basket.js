...
function validate(str){
  ...
  console.log("[VALIDATE] NUM1 " + num1)
  console.log("[VALIDATE] NUM2 " + num2)
  ...
  return (
    <div className="Place">
      ...
      <div className="Place__choice">
        ...
        <div className="Place__choice-item">
          ...
          <input
            ...
            onKeyUp={event => {
              console.log(time);
              setValidate(ValidateSTR(time))
              if (time=="") setFaster(true);
      ...
      <footer className="Place__footer">
        <Link to={`/order/${area.id}/${item.id}`} className={ (isValidate | faster)?'Place__order':'Place__order Link-disabled'} >
          Оплатить {price}
        </Link>
      </footer>
    </div>
